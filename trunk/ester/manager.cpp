...
void Manager::main() //{{{1
{
	...
	for (conf::Robot::const_iterator iter = m_a.begin() ; iter != m_a.end(); i++,iter++)
		m_drivers.push_back(new Task(drivers::getFactory(iter->c_str())(m_pa, i, driverDone)));

	for (conf::Robot::const_iterator iter = m_b.begin() ; iter != m_b.end(); i++,iter++)
		m_drivers.push_back(new Task(drivers::getFactory(iter->c_str())(m_pb, i, driverDone)));
